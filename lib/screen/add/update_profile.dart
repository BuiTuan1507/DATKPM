...
class _UpdateProfileState extends State<UpdateProfile> {
  ...
  Future uploadPic(BuildContext context) async {
    if (_image != null) {
      while (urlUser == null) {
        ...
        taskSnapshot.ref
            .getDownloadURL()
            .then((value) => {urlUser = value.toString()});
      }
      print("Upload ảnh thành công");
      Firestore.instance
          .collection("User")
          .document(widget.uuid)
          .updateData({"imageUser": urlUser});
      print(1234);
      print(urlUser);
      print(widget.uuid);
      showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Text('Bạn đã thay đổi ảnh thành công'),
              actions: [
                FlatButton(
                  child: Text("Ok"),
                  onPressed: () {
            ...
            child: Column(
              children: <Widget>[
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          showNameText(queryData, _user.name, () {
                            Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                          }),
                        ],
                      ),
                    )),
                ...
                (_user.email.length == 0)
                    ? (_showData('Thêm Email', () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      }))
                    : (_showIfData('Email', _user.email, () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      })),
                Divider(
                  color: Colors.grey[400],
                ...
                (_user.email.length == 0)
                    ? (_showData('Thêm ngày sinh của bạn', () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      }))
                    : (_showIfData('Ngày sinh của bạn', '30/02/1990', () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      })),
                Divider(
                  color: Colors.grey[400],
                ...
                (_user.address.length == 0)
                    ? (_showData('Thêm địa chỉ', () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      }))
                    : (_showIfData('Địa chỉ', _user.address, () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      })),
                Divider(
                  color: Colors.grey[400],
                ...
                (_user.sex == 0)
                    ? (_showData('Thêm giới tính', () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      }))
                    : (_showIfData('Giới tính', sexUser, () {
                        Navigator.pushNamed(context, CHANGEINFO,arguments: {'user':_user,'sex':_user.sex});
                      })),
                Divider(
                  color: Colors.grey[400],
  ...
  Widget _showData(String name, Function onPress) {
    // hien thi widget khi chua nhap du lieu
    return Row(
      children: <Widget>[
        InkWell(
          onTap: onPress,
