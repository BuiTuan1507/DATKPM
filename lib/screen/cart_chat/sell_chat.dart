...
class SellChat extends StatefulWidget {
  String uuid;
  SellChat({Key key, this.uuid}) : super(key: key);
  @override
  _SellChatState createState() => _SellChatState();
}

class _SellChatState extends State<SellChat> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_chat != null){
      for (int  i = 0 ; i< _chat.length ; i++){
        if (widget.uuid == _chat[i].sellUuid) {
            ...
            ListView.builder(
              ...
              itemBuilder: (context,index){
                ...
                for (int  i = 0 ; i < _store.length ; i++){
                  if ((_store[i].uuid == _chatRoom[index].buyUuid ) || (_store[i].uuid == _chatRoom[index].sellUuid)){
                    store = _store[i];
                    break;
                  }
                }
                ...
                for (int  i = 0; i< _userList.length ; i++){
                  if(_chatRoom[index].buyUuid == _userList[i].uuid){
